...
export default function Component() {
  ...
  useEffect(() => {
    ...
    if (isDetecting) {
      interval = setInterval(async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get_latest`);
        ...
        if (data.high_risk_reached) {
          ...
          await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/stop_detection`, { method: 'POST' });
        }
      }, 1000);
    }
  ...
  const handleStartStop = async () => {
    if (isDetecting) {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/start_detection`, { method: 'POST' });
    }
